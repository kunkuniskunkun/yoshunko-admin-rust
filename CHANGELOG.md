...
- 修复更新检查 ACL 权限缺失
- 修复 latest.json 版本号 v 前缀导致更新检测失败
- 手动检查更新增加错误提示
...
- 修复进入编辑页未修改仍标记脏状态的问题
- 新增编辑页返回仓库页的滑动动画
- 角色编辑页名称栏优化（中英文同行显示，ID 靠右）
- 防卫战·危局页面优化（修正错别字、卡片增加装饰元素、使用说明默认展开）
- 快速启动页卡片和状态栏添加透明度
- 设置页更新栏添加背景透明度
...
- 修复画廊返回闪白：画廊 DOM 始终保留，用 `gallery-hidden`（height:0）折叠，返回时 scrollTop 用 `requestAnimationFrame` 恢复
- 修复亮色模式下背景图卡片显示黑底：亮色用白色半透明，暗色保持深色

### 透明度调整

- 侧边栏 0.4、标题栏 0.5、UID 选择器 0.5、卡片 0.9（亮色 0.85 白底）、编辑器按钮栏 0.7、设置输入框 0.5
...
- 图片加载改用 base64 data URL，兼容所有 Tauri 配置
- 有背景图时侧边栏/标题栏（0.6）+ UID 选择器/编辑器按钮栏/设置输入框添加半透明 + 毛玻璃效果

**主题色修复**
- 修复暗色模式下自定义主题色不生效（CSS 优先级问题）
